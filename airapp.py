...
# ─────────────────────────────────────────────────────────────
#  NUCLEAR CSS + AURORA — injected into <head> and <body>
#  via JS to bypass Streamlit's inline style engine
# ─────────────────────────────────────────────────────────────
st.markdown("""
<script>
(function injectAll() {
  ...
  styleEl.id = 'skylens-nuclear-css';
  styleEl.textContent = `
    ...
    /* Force body to deep space so the canvas shows through everything */
    body {
      background: #04010d !important;
      background-color: #04010d !important;
    }
    ...
    [data-testid="stSidebar"] {
      ...
      background-color: rgba(4,1,13,0.88) !important;
    ...
    [data-testid="metric-container"] {
      ...
      border-left: 3px solid #00ffb3 !important;
      border-radius: 16px !important;
      padding: 24px 28px !important;
      box-shadow: 0 2px 16px rgba(0,0,0,0.55), 0 0 0 1px rgba(255,255,255,0.04), inset 0 1px 0 rgba(255,255,255,0.10) !important;
    ...
    [data-testid="metric-container"]:hover {
      ...
      box-shadow: 0 8px 40px rgba(0,0,0,0.65), 0 0 32px rgba(77,159,255,0.40), 0 0 80px rgba(77,159,255,0.15) !important;
    ...
    .stTabs [data-baseweb="tab"]:hover {
      ...
      background-color: rgba(0,255,179,0.08) !important;
    ...
    .stTabs [aria-selected="true"] {
      ...
      color: #00ffb3 !important;
      box-shadow: 0 0 32px rgba(0,255,179,0.40), 0 0 80px rgba(0,255,179,0.15) !important;
    ...
    [data-baseweb="popover"] [data-baseweb="menu"] {
      ...
      background-color: rgba(6,1,18,0.96) !important;
      backdrop-filter: blur(28px) !important;
      border: 1px solid rgba(179,102,255,0.22) !important;
      border-radius: 16px !important;
    }
    [data-baseweb="popover"] [role="option"] { color: rgba(255,255,255,0.78) !important; }
    [data-baseweb="popover"] [role="option"]:hover {
      background: rgba(0,255,179,0.10) !important;
      background-color: rgba(0,255,179,0.10) !important;
      color: #00ffb3 !important;
    }
    ...
    [data-testid="stDataFrame"]:hover {
      box-shadow: 0 8px 40px rgba(0,0,0,0.65), 0 0 32px rgba(179,102,255,0.40) !important;
    }
    ...
    [data-testid="stDataFrame"] tr:hover td {
      ...
      background-color: rgba(0,255,179,0.04) !important;
    }
    ...
    [data-testid="stForm"] {
      ...
      box-shadow: 0 8px 40px rgba(0,0,0,0.65), inset 0 1px 0 rgba(255,255,255,0.10) !important;
    }

    /* ── SUBMIT BUTTON ── */
    [data-testid="stFormSubmitButton"] > button {
      background: linear-gradient(135deg, rgba(77,159,255,0.90) 0%, rgba(179,102,255,0.80) 50%, rgba(224,64,251,0.75) 100%) !important;
      ...
      box-shadow: 0 4px 30px rgba(77,159,255,0.40) !important;
    }
    ...
    .stButton > button:hover {
      ...
      box-shadow: 0 0 32px rgba(0,255,179,0.40) !important;
      transform: translateY(-4px) scale(1.025) !important;
      color: #00ffb3 !important;
    }
    ...
    [data-testid="stAlert"] {
      ...
      background-color: rgba(77,159,255,0.06) !important;
      backdrop-filter: blur(12px) !important;
    }
    [data-testid="stAlert"] p { color: rgba(255,255,255,0.78) !important; }
    ...
    header[data-testid="stHeader"] {
      ...
      background-color: rgba(4,1,13,0.78) !important;
    ...
    .hero {
      ...
      position: relative;
      overflow: hidden;
      box-shadow: 0 24px 72px rgba(0,0,0,0.75), inset 0 1px 0 rgba(255,255,255,0.10);
    ...
    .hero::after {
      ...
      height: 1px;
      background: linear-gradient(90deg, transparent 0%, rgba(0,255,179,0.70) 25%, rgba(179,102,255,0.70) 55%, rgba(224,64,251,0.60) 80%, transparent 100%);
    ...
    .hero-glyph {
      ...
      font-size: 11rem;
      opacity: 0.045;
      background: linear-gradient(135deg, #00ffb3, #b366ff);
    ...
    .sidebar-brand span { color: #b366ff; }
    .section-hd {
      margin: 0 0 4px 0;
      font-size: 1.08rem;
      font-weight: 800;
    ...
    .info-box {
      ...
      background-color: rgba(77,159,255,0.07);
    ...
    .pill-expand   { background:rgba(0,255,179,0.14);  color:#00ffb3; border-color:rgba(0,255,179,0.45);  box-shadow:0 0 16px rgba(0,255,179,0.30); }
    .pill-maintain { background:rgba(245,230,66,0.14); color:#f5e642; border-color:rgba(245,230,66,0.45);  box-shadow:0 0 16px rgba(245,230,66,0.30); }
    .pill-optimize { background:rgba(255,107,53,0.14); color:#ff6b35; border-color:rgba(255,107,53,0.45);  box-shadow:0 0 16px rgba(255,107,53,0.30); }
    .pill-drop     { background:rgba(255,45,120,0.14); color:#ff2d78; border-color:rgba(255,45,120,0.45);  box-shadow:0 0 16px rgba(255,45,120,0.30); }
    .result-card {
      border: 1px solid rgba(255,255,255,0.10);
      border-radius: 30px;
      padding: 36px 44px;
    ...
    .result-expand   { background:rgba(0,255,179,0.07);  background-color:rgba(0,255,179,0.07);  border-left:4px solid #00ffb3; box-shadow:0 24px 72px rgba(0,0,0,0.75), 0 0 60px rgba(0,255,179,0.16); }
    .result-maintain { background:rgba(245,230,66,0.07); background-color:rgba(245,230,66,0.07); border-left:4px solid #f5e642; box-shadow:0 24px 72px rgba(0,0,0,0.75), 0 0 60px rgba(245,230,66,0.16); }
    .result-optimize { background:rgba(255,107,53,0.07); background-color:rgba(255,107,53,0.07); border-left:4px solid #ff6b35; box-shadow:0 24px 72px rgba(0,0,0,0.75), 0 0 60px rgba(255,107,53,0.16); }
    .result-drop     { background:rgba(255,45,120,0.07); background-color:rgba(255,45,120,0.07); border-left:4px solid #ff2d78; box-shadow:0 24px 72px rgba(0,0,0,0.75), 0 0 60px rgba(255,45,120,0.16); }
    .divider-label {
      display: flex;
      align-items: center;
      gap: 16px;
  ...
  /* ── 2. INJECT AURORA CANVAS INTO <BODY> ── */
  if (document.getElementById('skylens-aurora')) return;

  const canvas = document.createElement('canvas');
  canvas.id = 'skylens-aurora';
  ...
  /* Make sure the Streamlit app sits above the canvas */
  const ensureStack = () => {
    const app = document.getElementById('root') || document.querySelector('.stApp');
    if (app) {
  ...
  ensureStack();
  setTimeout(ensureStack, 500);
  setTimeout(ensureStack, 1500);
  ...
  const STAR_TINTS = [[255,255,255],[180,220,255],[200,180,255],[180,255,230]];
  ...
  function drawBand(b) {
    ...
    for (let i = 0; i <= pts; i++) {
      const x = (i / pts) * W;
      const ph = t * b.speed + i * 0.08;
      const y = baseY
        + Math.sin(ph * b.freq)                * b.amp * H
        + Math.sin(ph * b.freq * 1.618 + 1.2) * b.amp * H * 0.4
        + Math.sin(ph * b.freq * 0.618 + 2.5) * b.amp * H * 0.2;
    ...
    for (let i = pts; i >= 0; i--) {
      ...
      const ph = t * b.speed + i * 0.08;
      const y = baseY + thick
        + Math.sin(ph * b.freq + 0.5)          * b.amp * H * 0.6
        + Math.sin(ph * b.freq * 1.618 + 2.0)  * b.amp * H * 0.25;
      ctx.lineTo(x, y);
    }
    ctx.closePath();
  ...
  function frame() {
    ...
    stars.forEach(s => {
      ...
      if (s.r > 1.1 && twinkle > 0.82) {
        ...
        ctx.beginPath();
        ctx.moveTo(s.x * W - g, s.y * H); ctx.lineTo(s.x * W + g, s.y * H);
        ctx.moveTo(s.x * W, s.y * H - g); ctx.lineTo(s.x * W, s.y * H + g);
        ctx.stroke();
        ctx.restore();
      }
    });
...
with tab1:
    ...
    with left:
        ...
        wc = {"Expand": CHART_EXPAND, "Maintain": CHART_MAINTAIN, "Optimize": CHART_OPTIMIZE, "Drop": CHART_DROP}
        ...
        wedges, texts, autotexts = ax.pie(
            ...
            pctdistance=0.78, radius=0.90,
        )
        for at in autotexts:
            at.set_fontsize(8.5); at.set_color("#ffffff"); at.set_fontweight("bold")
        for txt in texts:
            txt.set_color((1, 1, 1, 0.75))
        theta = np.linspace(0, 2 * np.pi, 300)
        ax.plot(0.95 * np.cos(theta), 0.95 * np.sin(theta), color=(1,1,1,0.04), linewidth=8, zorder=0)
...
with tab2:
    ...
    with left:
        ...
        bars = ax.bar(apd.index, apd.values, color=colors, width=0.48, edgecolor="#07021a", linewidth=1.5, zorder=3)
        add_bar_glow(ax, bars, colors)
        mx = abs(apd.values).max() if len(apd) else 1
        for b in bars:
            h = b.get_height()
            ax.text(b.get_x() + b.get_width()/2, h + mx*0.028, f"${h:,.0f}",
                    ha="center", va="bottom", fontsize=8, color=(1,1,1,0.65), fontweight="bold")
    ...
    with right:
        ...
        bars = ax.bar(ald.index, ald.values, color=colors, width=0.48, edgecolor="#07021a", linewidth=1.5, zorder=3)
        add_bar_glow(ax, bars, colors)
        for b in bars:
            h = b.get_height()
    ...
    bars = ax.barh(cost_means.index, cost_means.values, color=bar_colors, edgecolor="#07021a", linewidth=1.5, height=0.52, zorder=3)
    mx = cost_means.max() if len(cost_means) else 1
    for b in bars:
        w = b.get_width()
        ax.text(w + mx*0.015, b.get_y() + b.get_height()/2, f"${w:,.0f}",
...
with tab3:
    ...
    st.markdown("<p style='font-size:0.78rem;font-weight:800;color:#ffffff;margin-bottom:10px;letter-spacing:0.04em'>Top routes currently classified as Expand</p>", unsafe_allow_html=True)
    expand_routes = (fdf[fdf["Route_Decision"] == "Expand"].sort_values("Profit", ascending=False)
    ...
    with left:
        st.markdown("<p style='font-size:0.78rem;font-weight:800;color:#ffffff;margin-bottom:10px'>Top 10 routes by total profit</p>", unsafe_allow_html=True)
        top = fdf.groupby("Route")["Profit"].sum().sort_values(ascending=True).tail(10)
        fig, ax = clean_fig((7, 5.2))
        bars = ax.barh(top.index, top.values, color=CHART_EXPAND, edgecolor="#07021a", linewidth=1.5, height=0.52, zorder=3)
        add_bar_glow(ax, bars, [CHART_EXPAND]*len(bars))
        ax.set_xlabel("Total Profit ($)", fontsize=8.5); ax.set_title("Top 10 Routes", pad=14)
        fig.tight_layout(); st.pyplot(fig); plt.close(fig)

    with right:
        st.markdown("<p style='font-size:0.78rem;font-weight:800;color:#ffffff;margin-bottom:10px'>Bottom 10 routes by total profit</p>", unsafe_allow_html=True)
        worst = fdf.groupby("Route")["Profit"].sum().sort_values(ascending=True).head(10)
        fig, ax = clean_fig((7, 5.2))
        bars = ax.barh(worst.index, worst.values, color=CHART_DROP, edgecolor="#07021a", linewidth=1.5, height=0.52, zorder=3)
        add_bar_glow(ax, bars, [CHART_DROP]*len(bars))
        ax.set_xlabel("Total Profit ($)", fontsize=8.5); ax.set_title("Bottom 10 Routes", pad=14)
        fig.tight_layout(); st.pyplot(fig); plt.close(fig)
...
with tab4:
    ...
    left, right = st.columns(2)
    with left:
        st.markdown("<p style='font-size:0.78rem;font-weight:800;color:#ffffff;margin-bottom:10px'>Routes with most decision changes</p>", unsafe_allow_html=True)
        st.dataframe(route_switches.head(10), use_container_width=True, hide_index=True)
    with right:
        st.markdown("<p style='font-size:0.78rem;font-weight:800;color:#ffffff;margin-bottom:10px'>Routes seen in the most decision states</p>", unsafe_allow_html=True)
    ...
    st.markdown("<p style='font-size:0.78rem;font-weight:800;color:#ffffff;margin-bottom:10px'>How many unique routes appear in each category?</p>", unsafe_allow_html=True)

    urbd = (df.groupby("Route_Decision")["Route"].nunique().reset_index()
            .rename(columns={"Route":"Unique Routes"}).set_index("Route_Decision")
            .reindex(ORDER).dropna())
    bc_map = {"Expand":CHART_EXPAND,"Maintain":CHART_MAINTAIN,"Optimize":CHART_OPTIMIZE,"Drop":CHART_DROP}
    bc = [bc_map.get(i, "#CCCCCC") for i in urbd.index]
    fig, ax = clean_fig((8, 4.2))
    bars = ax.bar(urbd.index, urbd["Unique Routes"], color=bc, width=0.48, edgecolor="#07021a", linewidth=1.5, zorder=3)
    add_bar_glow(ax, bars, bc)
    for b in bars:
        h = b.get_height()
        ax.text(b.get_x()+b.get_width()/2, h+0.4, f"{int(h)}", ha="center", fontsize=9, color=(1,1,1,0.65), fontweight="bold")
    ...
    ax2.legend(handles=patches, loc="center", frameon=False, ncol=4, prop={"size":9}, labelcolor="#ffffff")
...
with tab5:
    ...
    st.markdown("<p style='font-size:0.78rem;font-weight:800;color:#ffffff;margin-bottom:10px'>How accurate are the models?</p>", unsafe_allow_html=True)
    fig, ax = clean_fig((8, 3.8))
    x = np.arange(len(comparison)); w = 0.30
    b1 = ax.bar(x-w/2, comparison["Accuracy"], w, label="Accuracy", color=COLOR_ACCENT, edgecolor="#07021a", linewidth=1.5, zorder=3)
    b2 = ax.bar(x+w/2, comparison["Macro F1"], w, label="Macro F1", color=COLOR_AMETHYST, edgecolor="#07021a", linewidth=1.5, zorder=3)
    add_bar_glow(ax, list(b1), [COLOR_ACCENT]*len(b1))
    add_bar_glow(ax, list(b2), [COLOR_AMETHYST]*len(b2))
    for b in list(b1)+list(b2):
        h = b.get_height()
        ax.text(b.get_x()+b.get_width()/2, h+0.007, f"{h:.0%}", ha="center", fontsize=7.5, color=(1,1,1,0.60), fontweight="bold")
    ax.set_xticks(x); ax.set_xticklabels(comparison["Model"], fontsize=8.5)
    ax.set_ylim(0, 1.18); ax.set_ylabel("Score", fontsize=8.5)
    ax.set_title("Model Accuracy & F1 Comparison", pad=14)
    leg = ax.legend(frameon=False, fontsize=9)
    for text in leg.get_texts(): text.set_color((1,1,1,0.65))
    plt.xticks(rotation=10); fig.tight_layout(); st.pyplot(fig); plt.close(fig)
    ...
    model_choice = st.selectbox("Prediction mode",
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            st.markdown("<p style='font-size:0.65rem;font-weight:900;color:rgba(255,255,255,0.38);letter-spacing:0.14em;text-transform:uppercase;margin-bottom:14px;font-family:\"JetBrains Mono\",monospace'>Flight basics</p>", unsafe_allow_html=True)
        ...
        with col2:
            st.markdown("<p style='font-size:0.65rem;font-weight:900;color:rgba(255,255,255,0.38);letter-spacing:0.14em;text-transform:uppercase;margin-bottom:14px;font-family:\"JetBrains Mono\",monospace'>Route context</p>", unsafe_allow_html=True)
        ...
        if model_choice == "With Revenue Variables":
            with col3:
                st.markdown("<p style='font-size:0.65rem;font-weight:900;color:rgba(255,255,255,0.38);letter-spacing:0.14em;text-transform:uppercase;margin-bottom:14px;font-family:\"JetBrains Mono\",monospace'>Revenue</p>", unsafe_allow_html=True)
            ...
            with col3:
                st.markdown("<p style='font-size:0.65rem;font-weight:900;color:rgba(255,255,255,0.38);letter-spacing:0.14em;text-transform:uppercase;margin-bottom:14px;font-family:\"JetBrains Mono\",monospace'>Cost breakdown</p>", unsafe_allow_html=True)
    ...
    if st.session_state.pred_result is not None:
        ...
        text_col   = {"Expand":COLOR_GREEN,"Maintain":COLOR_YELLOW,"Optimize":COLOR_ORANGE,"Drop":COLOR_PINK}
        explanations = {
        ...
        <div class='result-card {result_cls.get(pred, "result-expand")}'>
          <div style='margin-bottom:14px'><span class='pill {pill_cls.get(pred, "")}'>{pred}</span></div>
          <div style='font-family:"Outfit",sans-serif;font-size:1.80rem;font-weight:900;color:{text_col.get(pred,"#111")};margin-bottom:14px;letter-spacing:-0.03em;text-shadow:0 0 40px {text_col.get(pred,"#111")}66'>
            Suggested Decision: {pred}
          </div>
          <p style='color:rgba(255,255,255,0.62);margin:0;font-size:0.92rem;line-height:1.78'>{explanations.get(pred,"")}</p>
        </div>
        """, unsafe_allow_html=True)

        st.markdown("<p style='font-size:0.78rem;font-weight:800;color:#ffffff;margin-bottom:10px'>Confidence by decision option</p>", unsafe_allow_html=True)
        prob_df = pd.DataFrame({"Decision":cls,"Probability":prob}).sort_values("Probability", ascending=False)
        bar_cp  = {"Expand":CHART_EXPAND,"Maintain":CHART_MAINTAIN,"Optimize":CHART_OPTIMIZE,"Drop":CHART_DROP}
        colors_prob = [bar_cp.get(d,"#CCCCCC") for d in prob_df["Decision"]]
        fig, ax = clean_fig((6, 3.8))
        bars = ax.bar(prob_df["Decision"], prob_df["Probability"], color=colors_prob, width=0.44, edgecolor="#07021a", linewidth=1.5, zorder=3)
        add_bar_glow(ax, bars, colors_prob)
        for b in bars:
            h = b.get_height()
            ax.text(b.get_x()+b.get_width()/2, h+0.014, f"{h:.0%}", ha="center", fontsize=9, color=(1,1,1,0.70), fontweight="bold")
        ax.set_ylim(0, 1.20); ax.set_ylabel("Probability", fontsize=8.5)
...
<div style='text-align:center;color:rgba(255,255,255,0.28);font-size:0.70rem;
            padding:44px 0 22px;font-family:"JetBrains Mono",monospace;letter-spacing:0.10em'>
