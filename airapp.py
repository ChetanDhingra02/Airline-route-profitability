...
# Local upload fallback: ChatGPT sometimes saves duplicate uploads with "(1)" in the filename.
if not os.path.exists(DATA_PATH):
    fallback_data_path = os.path.join(BASE_DIR, "airline_route_profitability(1).csv")
...
def prepare_input(input_df, training_columns):
    """One-hot encode a single scenario and align it exactly to the training columns."""
    enc = pd.get_dummies(input_df, drop_first=True)
    aligned = enc.reindex(columns=list(training_columns), fill_value=0)
    return aligned.astype(float)


def build_feature_frame(df_source, training_columns, include_revenue=False, include_costs=False):
    """Build a model-ready matrix from the app dataset using only columns used by that model."""
    base_cols = [
        "Aircraft_Type", "Aircraft_Capacity", "Passengers", "Load_Factor",
        "Flight_Hours", "Season", "Route_Category", "Demand_Level",
    ]
    revenue_cols = ["Ticket_Revenue", "Ancillary_Revenue"] if include_revenue else []
    cost_cols = [
    ...
    raw_cols = [c for c in base_cols + revenue_cols + cost_cols if c in df_source.columns]
    return prepare_input(df_source[raw_cols].copy(), training_columns)


def model_quality_table(df_source, model1, model2, model3, X1_columns, X2_columns, X3_columns):
    """Compute model metrics from the current dataset instead of hardcoding them in the app."""
    y = df_source["Route_Decision"]
    specs = [
        ("With revenue variables", model1, X1_columns, True, False),
        ("Without revenue variables", model2, X2_columns, False, True),
        ("Only pre-operational features", model3, X3_columns, False, False),
    ]
    rows = []
    for label, model, cols, inc_rev, inc_cost in specs:
        X = build_feature_frame(df_source, cols, include_revenue=inc_rev, include_costs=inc_cost)
...
# ─────────────────────────────────────────────────────────────
comparison = model_quality_table(df, model1, model2, model3, X1_columns, X2_columns, X3_columns)

df_sorted = df.sort_values(["Route", "Flight_Date"]).copy()
df_sorted["Prev_Decision"] = df_sorted.groupby("Route")["Route_Decision"].shift(1)
df_sorted["Changed"] = (
    df_sorted["Prev_Decision"].notna()
    & (df_sorted["Route_Decision"] != df_sorted["Prev_Decision"])
)
route_switches = (
...
with tab5:
    ...
    st.pyplot(fig)
    st.caption("Metrics are recomputed from the app dataset and saved models at load time, so this chart stays aligned with the current model files.")
    ...
    # Prevent stale results from a previous prediction mode being shown as if they belong to this mode.
    if st.session_state.get("pred_model_choice") != model_choice:
        st.session_state.pred_result = None
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            ...
            st.markdown(
                ...
                f"<strong>Auto load factor:</strong> {load_factor:.0%}<br>"
                f"Calculated as passengers ÷ aircraft capacity, so the model cannot receive contradictory inputs."
                f"</div>",
                unsafe_allow_html=True,
            )
            if passengers > aircraft_capacity:
                st.warning("Passengers exceed aircraft capacity, so load factor is capped at 100%. Please check the scenario.")
    ...
    if (
        st.session_state.pred_result is not None
        and st.session_state.pred_result.get("model_choice") == model_choice
    ):
...
# CHART_OPTIMIZE / CHART_DROP and CHART_NEUTRAL at the top of the file.